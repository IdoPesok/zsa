...
If the error data is an instance of the built-in `Error` class, the `ZSAError` instance will inherit the `message`, `stack`, `name`, and `cause` properties from the error data.
