...
export default function UseActionStateExample() {
  let [messages, submitAction] = useActionState( // [!code highlight]
...
ZSA supports file and/or image uploads within forms. To handle file uploads, you need to specify the expected file type in the server action using `z.custom<File>()` and include a file input in your form.
...
export const uploadFile = createServerAction()
  .input(
    z.object({
      name: z.string().min(5),
      file: z.custom<File>().refine((file) => file.size > 0), // [!code highlight]
    }),
    {
      type: "state",
    }
