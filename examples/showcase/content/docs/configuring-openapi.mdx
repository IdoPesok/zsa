...
By default, `zsa-openapi` will only accept requests with the `application/json` content type. If you need to accept other content types, you can specify them in the `contentTypes` option:
...
`zsa-openapi` will only allow requests through if the `content-type` header matches one of the specified content types.
