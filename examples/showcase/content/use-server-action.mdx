...
- `transition`: A function to wrap your action in a useTransition startTransition function.
- `isTransitioning`: A boolean indicating if your server action currently being ran.
...
Often when using RSC your server actions will need to invoke revalidatePath or redirect.  Calling those functions require you to manually call your actions inside transitions:
...
To make this easier, useServerAction provides a `transition({ /* input */})` function that wraps your action in a startTransition function.  This allows you to call your action like this:

```tsx
function MyComponent() {
  const { isTransitioning, transition } = useServerAction(myServerAction, {
    onSuccess: ({ data }) => {
...
<Warning>
The transition function will not return the results or errors of the action in a promise; therefore, you'll need to destruct data and error from the useServerAction hook, or use the onSuccess or onError callbacks to run logic after the action finishes such as showing toasts or alerts.
</Warning>