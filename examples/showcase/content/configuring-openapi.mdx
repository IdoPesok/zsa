...
```typescript:app/api/:postId/route.ts
import { z } from "zod"
import { createServerAction } from "zsa"
...
export const { GET } = createRouteHandlersForAction(getPostAction) <|highlight|>
```
...
```typescript:app/api/calculations/multiply/:number1/:number2/route.ts
import { z } from "zod"
import { createServerAction } from "zsa"
import { createRouteHandlersForAction } from "zsa-openapi"
...
const getPostAction = createServerAction()
  .input(
    z.object({
      number1: z.coerce.number(), <|highlight|>
      number2: z.coerce.number(), <|highlight|>
    })
  )
  .handler(async ({ input }) => {
    return {
...
const action = createServerAction().handler(async () => {
  ...
  return new Response(stream) <|highlight|>
})
