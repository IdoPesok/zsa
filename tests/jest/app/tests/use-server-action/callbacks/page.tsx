...
export default function CallbacksUI() {
    ...
    const { execute } = useServerAction(callbacksAction, {
        onStart: () => setCallbackData((prev) => ({ ...prev, onStart: CLIENT_TEST_DATA.dummyMessage })),
        onSuccess: () => setCallbackData((prev) => ({ ...prev, onSuccess: CLIENT_TEST_DATA.dummyMessage })),
        onError: () => setCallbackData((prev) => ({ ...prev, onError: CLIENT_TEST_DATA.dummyMessage })),
    })
